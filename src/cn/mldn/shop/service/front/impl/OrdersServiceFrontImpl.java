...
public class OrdersServiceFrontImpl implements IOrdersServiceFront {
	...
	public boolean insert(String mid)
		...
		try {
			//1.首先应该判断出当前用户信息是否完整，要根据mid查询出一个用户的完整信息
			...
			if(member.getName() == null || member.getPhone() == null || member.getAddress() == null) {
				throw new UnCompleteMemberInformationException("用户个人信息不完整，无法进行订单创建！");
			}
			...
			//4.检测购物车中是否有购买商品的信息
			if(cars.size() == 0) {
				throw new EmptyShopcarException("购物车为空，暂时无法创建订单！");
			}
			//5.如果现在要是有购物信息，则需要查询出所有的商品信息，目的是取出商品的名称、价格、库存量
			...
			while(iterGoods.hasNext()) {
				...
				pay += vo.getPrice() * cars.get(vo.getGid());  //单价*数量
			}
			//8.创建订单对象
			...
			//11.修改所有购买商品的数量以及创建所有详情对象
			while(iterGoods.hasNext()) {
				Details vo = new Details();
				Goods goods = iterGoods.next();
				vo.setGoods(goods);  //设置商品ID
				vo.setOrders(orders);  // 设置订单ID
	...
	public Orders show(String mid, int oid) throws Exception {
		try {
			Orders vo = DAOFactory.getIOrdersDAOInstance(this.dbc.getConnection()).findAllByIdAndMember(mid, oid);
			if(vo != null) {
